...
const assertSafeCookieAttribute = (name: string, value: string): void => {
	if (/[;\r\n]/.test(value)) {
		throw new Error(`Invalid cookie attribute "${name}": must not contain ";" or newlines`);
	}
};
...
const setCookie = (cookieName: string, value: string, options: CookieAttributes = {}) => {
	...
	if (options.path) {
		assertSafeCookieAttribute('path', options.path);
		cookie += `; path=${options.path}`;
	}
	if (options.domain) {
		assertSafeCookieAttribute('domain', options.domain);
